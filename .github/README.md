# MiniLesson — Composer mirror

This repository is a **Composer/Packagist mirror** of the Moodle plugin
[`mod_minilesson`](https://moodle.org/plugins/mod_minilesson). The plugin's own files
...
| | |
|---|---|
| **Package** | `udir-moodle/mod_minilesson` (type `moodle-mod`) |
| **Installs to** | the Moodle path for that type, via `composer/installers` (folder `minilesson`) |
...
```bash
composer require 'udir-moodle/mod_minilesson:*'   # '*' — versions are Moodle date integers
```
...
"extra": { "installer-paths": {
    ...
    "exact/dir/":           ["udir-moodle/mod_minilesson"]  // only this package
} }
```
